...
// go run chat_room.go
//
// You can switch between implementations by setting the USE_HIGH_LEVEL environment variable:
// USE_HIGH_LEVEL=true go run chat_room.go    # Uses high-level WebSocket features
// go run chat_room.go                         # Uses low-level implementation (default)
...
func main() {
	...
	RunChatRoomExample(useHighLevel)
}

// RunChatRoomExample is the main entry point that chooses between implementations
func RunChatRoomExample(useHighLevel bool) {
	if useHighLevel {
...
func runHighLevelChatRoom() {
	...
	hub.OnConnect(func(ctx context.Context, client router.WSClient, request any) error {
		// Get username from query parameter (the hub passes the Context as request)
		username := "Guest" + client.ID()[:8] // Default username
		if reqCtx, ok := request.(router.Context); ok {
			if q := reqCtx.Query("username"); q != "" {
				username = q
			}
		}
	...
	// Handle JSON messages using the On method for event-based messaging
	hub.On("message", func(ctx context.Context, client router.WSClient, data any) error {
		// Parse the incoming message
		msgData, ok := data.(map[string]any)
		if !ok {
		...
		// Add to history and broadcast
		chatRoom.mu.Lock()
		chatRoom.history = append(chatRoom.history, msg)
		if len(chatRoom.history) > chatRoom.maxHistory {
			chatRoom.history = chatRoom.history[1:]
		}
		chatRoom.mu.Unlock()
	...
	hub.On("typing", func(ctx context.Context, client router.WSClient, data any) error {
		username := client.Get("username")
